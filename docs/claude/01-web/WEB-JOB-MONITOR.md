...
#### 1.2.3 Sequence Management
- ✅ Job-specific sequence tracking (Map<jobId, lastSequence>)
  ...
  setJobs(prev => {
    ...
    return prev.map(job =>
      job.jobId === data.jobId
        ? { ...job, progress: { current, total, percentage }, metadata: { phase: 'crawl' } }
        : job
    );
  });
...
**Problem**: Socket.io events can arrive out-of-order due to network latency.

**Example**:
```
Server sends: #1 → #2 → #3
Client receives: #1 → #3 → #2 (out of order!)
```

Without sequence tracking, progress could regress (e.g., 80% → 50%).
...
/**
 * Check if event is newer than last processed event
 */
const checkSequence = useCallback((jobId: string, newSequence: number): boolean => {
  ...
  if (newSequence < lastSequence) {
    ...
    return false; // Ignore outdated event
  }
...
/**
 * Reset sequence on job completion
 */
...
newSocket.on('review:completed', (data: any) => {
  resetSequence(data.jobId); // ✅ Clean up memory
  setJobs(prev => /* mark completed */);
});
```
...
- [`SHARED-HOOKS.md`](../../03-shared/SHARED-HOOKS.md) - useSocket hook
...
2. **Sequence management**: Out-of-order event handling
...
**Key Innovation**: Two-phase new job detection:
1. Global `job:new` event → all clients notified
2. Auto-subscribe to restaurant room → receive progress events
3. Defensive job creation in progress handlers → resilient to missed events

This ensures **zero missed jobs** and **real-time monitoring** across all restaurants.
